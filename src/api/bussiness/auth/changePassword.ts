 ...
 * @param business_id - UUID del customer
 * @returns Objeto con allowed (boolean) y remainingTime (number en ms)
 */
function checkRateLimit(business_id: string): { allowed: boolean; remainingTime: number } {
  const key = `password_change_${business_id}`;
  const now = Date.now();
 ...
 */
function recordAttempt(business_id: string, success: boolean = false): void {
  const key = `password_change_${business_id}`;
  const now = Date.now();
...
export const changePasswordController = async (req: Request, res: Response): Promise<Response> => {
  const { business_id, currentPassword, newPassword } = req.body as {
    business_id: string;
  ...
  console.log(`🔐 Intento de cambio de contraseña para business_id: ${business_id} - ${new Date().toISOString()}`);

  // Verificar rate limiting
  const rateLimitCheck = checkRateLimit(business_id);
  if (!rateLimitCheck.allowed) {
    const remainingMinutes = Math.ceil(rateLimitCheck.remainingTime / (1000 * 60));
    console.log(`🚫 Rate limit excedido para ${business_id}. Tiempo restante: ${remainingMinutes} minutos`);
  ...
  if (!business_id || !currentPassword || !newPassword) {
    recordAttempt(business_id, false);
    return res.status(400).json({
      success: false,
      message: 'business_id, contraseña actual y nueva contraseña son obligatorios.',
    });
  }
  ...
  try {
    // Buscar al usuario en la base de datos por business_id
    ...
    if (fetchError || !user) {
      recordAttempt(business_id, false);
      console.log(`❌ Usuario no encontrado: ${business_id}`);
    ...
    if (!isValidCurrentPassword) {
      recordAttempt(business_id, false);
      console.log(`❌ Contraseña actual incorrecta para: ${business_id}`);
    ...
    // Registrar éxito
    recordAttempt(business_id, true);
    console.log(`✅ Contraseña cambiada exitosamente para: ${user.business_email}`);

    // Enviar notificación por email (no bloquear si falla)
    try {
      console.log(`📧 Intentando enviar notificación de cambio de contraseña a: ${user.business_email}`);
    ...
    recordAttempt(business_id, false);
    console.error(`💥 Error al cambiar contraseña para ${business_id}:`, error);
